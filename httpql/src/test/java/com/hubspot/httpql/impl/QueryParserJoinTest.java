...
import com.hubspot.httpql.model.EntityWithJoin;

public class QueryParserJoinTest {

  private Multimap<String, String> query;
  private QueryParser<EntityWithJoin> parser;

  @Before
  public void setup() {
    query = ArrayListMultimap.create();
    parser = QueryParser.newBuilder(EntityWithJoin.class).build();
  }
  ...
  public void joinWithSingleBoundValue() {
    ...
    ParsedQuery<EntityWithJoin> parsedQuery = parser.parse(query);
    assertThat(parsedQuery.getBoundQuery().getTopicId()).isEqualTo(123L);

    SelectBuilder<EntityWithJoin> selectBuilder = SelectBuilder.forParsedQuery(parsedQuery);
