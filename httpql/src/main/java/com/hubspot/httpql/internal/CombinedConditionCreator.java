...
public class CombinedConditionCreator<T extends QuerySpec>
  ...
  public CombinedConditionCreator(
    Operator operator,
    Collection<FilterEntryConditionCreator<T>> conditionCreators
  ) {
  ...
  public boolean removeAllFiltersFor(String fieldName) {
    boolean removedFromThis = conditionCreators.removeIf(f ->
      ...
      ((BoundFilterEntry<T>) f).getQueryName().equals(fieldName)
    );
