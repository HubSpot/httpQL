...
public class MultiValuedBoundFilterEntry<T extends QuerySpec>
  extends BoundFilterEntry<T> {

  private final Collection<?> values;

  public MultiValuedBoundFilterEntry(
    BoundFilterEntry<T> boundFilterEntry,
    Collection<?> values
  ) {
    super(
  ...
  public Condition getCondition(QuerySpec value, FieldFactory fieldFactory) {
    return getConditionProvider(fieldFactory)
      .getCondition(values, getProperty().getName());
  }
