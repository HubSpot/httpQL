...
public class JsonGreaterThanOrEqualImpl extends JsonFilterBase implements FilterImpl {
    ...
    return new MultiParamConditionProvider<>(field) {
      ...
      public Condition getCondition(Collection<T> values) {
        Preconditions.checkArgument(
          values.size() == 2,
          "JsonGreaterThanOrEqual filters require exactly 2 parameters"
        );
        JsonFilterParts jsonFilterParts = getJsonFilterParts(field, values);
        return jsonFilterParts
          .fieldValue.cast(BigDecimal.class)
          .greaterOrEqual(new BigDecimal(jsonFilterParts.filterValues.get(0).data()));
      }
    };
  }
  ...
  public Set<Class<? extends Filter>> getAnnotationClasses() {
    return ImmutableSet.of(
      ...
      com.hubspot.httpql.filter.JsonGreaterThanOrEqual.class
    );
  }
}
