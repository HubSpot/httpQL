...
public class JsonIsNotDistinctFromImpl extends JsonFilterBase implements FilterImpl {
    ...
    return new MultiParamConditionProvider<T>(field) {
      ...
      public Condition getCondition(Collection<T> values) {
        Preconditions.checkArgument(
          values.size() == 2,
          "JsonIsNotDistinctFrom filters require exactly 2 parameters"
        );
        JsonFilterParts jsonFilterParts = getJsonFilterParts(field, values);
        return jsonFilterParts.fieldValue.isNotDistinctFrom(
          jsonFilterParts.filterValues.get(0)
        );
      }
    };
  }
  ...
  public Set<Class<? extends Filter>> getAnnotationClasses() {
    return ImmutableSet.of(
      ...
      com.hubspot.httpql.filter.JsonIsNotDistinctFrom.class
    );
  }
}
