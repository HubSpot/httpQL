...
import com.hubspot.httpql.lib.filter.EqualImpl;
...
public class UriParamParser {
  ...
  public static Optional<Class<? extends FilterIF>> getFilterByName(String name) {
    ...
    return Optional.of(BY_NAME.get(name).getAnnotationClass());
  }
  ...
  public ParsedUriParams parseUriParams(Map<String, List<String>> uriParams, boolean allowDoubleUnderscoreInFieldName) {
    ...
    for (Map.Entry<String, List<String>> entry : params.entrySet()) {
      ...
      final FilterImpl filter = BY_NAME.get(filterName);
  ...
  private static String fieldNameFromParts(List<String> parts, boolean allowDoubleUnderscoreInFieldName) {
    ...
    if (BY_NAME.containsKey(parts.get(parts.size() - 1))) {
